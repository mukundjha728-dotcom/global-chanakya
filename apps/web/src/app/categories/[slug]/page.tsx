...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data = await EntityService.getEntityData("Category", decodedSlug, "categoryId");
  ...
  return generateSeoMetadata({
    title,
    description,
    canonicalUrl: `${SITE_URL}/categories/${decodedSlug}`,
    type: "website",
...
export default async function CategoryHubPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articles = await EntityService.getEntityArticles("categoryId", data.entity._id);
