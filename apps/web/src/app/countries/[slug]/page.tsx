...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data = await EntityService.getEntityData("Country", decodedSlug, "countries");
  ...
  return generateSeoMetadata({
    title,
    description,
    canonicalUrl: `${SITE_URL}/countries/${decodedSlug}`,
    type: "website",
...
export default async function CountryHubPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const decodedSlug = decodeURIComponent(slug);
  ...
  const articles = await EntityService.getEntityArticles("countries", data.entity._id);
  const relatedEntities = await EntityService.getContextualRelatedEntities("countries", data.entity._id);

  // Determine structured data
  const isProfile = ["Leader", "Organization"].includes("Country");
  ...
  const structuredData = {
    ...
    url: `${SITE_URL}/countries/${decodedSlug}`,
    mainEntity: isProfile ? {
      "@type": "Country" === "Leader" ? "Person" : "Organization",
      name: data.entity.name,
