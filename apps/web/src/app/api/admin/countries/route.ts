...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const doc = await Country.findByIdAndUpdate(id, { $set: updates }, { new: true }).lean();
    if (!doc) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[PATCH /api/admin/countries]", err);
    return NextResponse.json({ error: errorMsg(err) }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await Country.findByIdAndUpdate(id, { isDeleted: true, deletedAt: new Date() });
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[DELETE /api/admin/countries]", err);
    return NextResponse.json({ error: errorMsg(err) }, { status: 500 });
  }
}
