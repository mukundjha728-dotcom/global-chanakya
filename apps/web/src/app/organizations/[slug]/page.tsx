...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data = await EntityService.getEntityData("Organization", decodedSlug, "organizations");
  ...
  return generateSeoMetadata({
    title,
    description,
    canonicalUrl: `${SITE_URL}/organizations/${decodedSlug}`,
    type: "website",
...
export default async function OrganizationHubPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articles = await EntityService.getEntityArticles("organizations", data.entity._id);
