...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data = await EntityService.getEntityData("Leader", decodedSlug, "leaders");
  ...
  return generateSeoMetadata({
    title,
    description,
    canonicalUrl: `${SITE_URL}/leaders/${decodedSlug}`,
    type: "website",
...
export default async function LeaderHubPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const decodedSlug = decodeURIComponent(slug);
  ...
  const articles = await EntityService.getEntityArticles("leaders", data.entity._id);
  const relatedEntities = await EntityService.getContextualRelatedEntities("leaders", data.entity._id);

  // Determine structured data
  const isProfile = ["Leader", "Organization"].includes("Leader");
  ...
  const structuredData = {
    ...
    url: `${SITE_URL}/leaders/${decodedSlug}`,
    mainEntity: isProfile ? {
      "@type": "Leader" === "Leader" ? "Person" : "Organization",
      name: data.entity.name,
