...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data = await EntityService.getEntityData("Region", decodedSlug, "regions");
  ...
  return generateSeoMetadata({
    title,
    description,
    canonicalUrl: `${SITE_URL}/regions/${decodedSlug}`,
    type: "website",
...
export default async function RegionHubPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const decodedSlug = decodeURIComponent(slug);
  ...
  const articles = await EntityService.getEntityArticles("regions", data.entity._id);
