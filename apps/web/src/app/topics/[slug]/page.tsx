...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data = await EntityService.getEntityData("Topic", decodedSlug, "topics");
  ...
  return generateSeoMetadata({
    title,
    description,
    canonicalUrl: `${SITE_URL}/topics/${decodedSlug}`,
    type: "website",
...
export default async function TopicHubPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articles = await EntityService.getEntityArticles("topics", data.entity._id);
